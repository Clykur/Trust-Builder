  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center pt-24 pb-16 overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-5 text-center max-w-5xl">
        ...
        >
          Legal contracts. Zero-loss escrow. AI-verified delivery. Blockchain
          reputation.
          <span className="font-calegar uppercase tracking-tight">
            FreeTrust
          </span>{" "}
          eliminates counterparty risk from every professional engagement.
        </motion.p>
        ...
        >
          {stats.map((stat, i) => (
            <div
              key={i}
              className="bg-background/80 backdrop-blur-sm py-5 px-3 flex flex-col items-center gap-1"
            >
              <span className="text-xl md:text-2xl font-bold font-display text-foreground">
                {stat.value}
              </span>
              <span className="text-[10px] md:text-xs text-muted-foreground font-medium uppercase tracking-wider text-center">
                {stat.label}
              </span>
            </div>
          ))}
